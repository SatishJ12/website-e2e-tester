<#
.SYNOPSIS
    Same automation as Run-WebsiteE2ETests.ps1 (login, configured pages,
    logout, header/subheader checks, full-page + footer screenshots, Word
    report) - only the EXCEL layout is different. This variant is for an
    audit-style record: Site_Name, RowType, URL, Status (Pass/Fail/Error),
    Start_Time, End_Time, Duration (per website, not per page), Error.
...
.USAGE
    .\Run-SiteAuditReport.ps1 -ConfigPath .\config\Site-Config.csv
    Prompts for username/password every run, for every site - nothing is
    saved to disk between runs. Uses the same config file format as
    Run-WebsiteE2ETests.ps1 - no separate config needed.
#>

param(
    [string]$ConfigPath   = "$PSScriptRoot\..\config\Site-Config.csv",
    [string]$OutputFolder = "$PSScriptRoot\..\AuditRun_$(Get-Date -Format yyyyMMdd_HHmmss)",
...
# throw and must never affect Pass/Fail/Error status.
...
foreach ($group in $AllRows | Group-Object SiteName) {
    ...
    # Buffered per-site so the "Duration (per website)" figure can be
    # computed once processing for this site finishes, then stamped onto
    # every row this site produced, before they're added to $Results.
    $siteResultsBuffer = New-Object System.Collections.Generic.List[object]

    if (-not $loginRow) {
        $now = Get-Date
        $Results.Add([PSCustomObject]@{
            Site=$siteName; Page="CONFIG"; URL=""; Status="Error"
            Error="No Login row found in config"; StartTime=$now; EndTime=$now; Timestamp=$now
            Screenshot=""; DurationSec=0; SiteDurationSec=0; CapturedInfo=""; FooterScreenshot=""
        })
        continue
    }
    ...
    if ($loginRow.Enabled -eq 'No') {
        ...
        $Results.Add([PSCustomObject]@{
            ...
            Error=""; StartTime=$now; EndTime=$now; Timestamp=$now
            Screenshot=""; DurationSec=0; SiteDurationSec=0; CapturedInfo=""; FooterScreenshot=""
        })
        continue
    }
    ...
    $siteProcessStart = Get-Date
        ...
        $loginStatus = if ($postLoginEl) { "Pass" } else { "Fail" }
        ...
        $loginDuration = [math]::Round(($loginEndTime - $loginStepStart).TotalSeconds, 1)
        Write-Host "  [$siteName] LOGIN - $loginStatus ($loginDuration sec)"

        $siteResultsBuffer.Add([PSCustomObject]@{
            Site=$siteName; Page="LOGIN"; URL=$loginRow.Url; Status=$loginStatus
            Error=$(if ($loginStatus -eq 'Fail') { "Post-login element not found: $($loginRow.PostLoginCheckSelector)" } else { "" })
            StartTime=$loginStepStart; EndTime=$loginEndTime; Timestamp=$loginEndTime
            Screenshot=$loginShot; DurationSec=$loginDuration; SiteDurationSec=0; CapturedInfo=$loginCapturedInfo; FooterScreenshot=$loginFooterShot
        })

        if ($loginStatus -eq "Fail") { continue }   # skip pages if login didn't work

        # ---- PAGES ----
        foreach ($page in $pageRows) {
            $status = "Pass"; $errMsg = ""; $capturedInfo = ""; $footerShot = ""
            ...
            try {
                ...
                if ($status -eq "Pass" -and $page.ExpectedSubHeader -and $page.SubHeaderSelector) {
            ...
            $pageEndTime = Get-Date
            $pageDuration = [math]::Round(($pageEndTime - $pageStepStart).TotalSeconds, 1)
            ...
            $siteResultsBuffer.Add([PSCustomObject]@{
                ...
                Error=$errMsg; StartTime=$pageStepStart; EndTime=$pageEndTime; Timestamp=$pageEndTime
                Screenshot=$shotPath; DurationSec=$pageDuration; SiteDurationSec=0; CapturedInfo=$capturedInfo; FooterScreenshot=$footerShot
            })
        }
        ...
        if ($loginRow.LogoutSelector) {
            ...
            try {
                ...
                $logoutEndTime = Get-Date
                $logoutDuration = [math]::Round(($logoutEndTime - $logoutStepStart).TotalSeconds, 1)
                Write-Host "  [$siteName] LOGOUT - Pass ($logoutDuration sec)"

                $siteResultsBuffer.Add([PSCustomObject]@{
                    Site=$siteName; Page="LOGOUT"; URL=$driver.Url; Status="Pass"
                    Error=""; StartTime=$logoutStepStart; EndTime=$logoutEndTime; Timestamp=$logoutEndTime
                    Screenshot=$logoutShot; DurationSec=$logoutDuration; SiteDurationSec=0; CapturedInfo=""; FooterScreenshot=""
                })
            } catch {
                $logoutEndTime = Get-Date
                $logoutDuration = [math]::Round(($logoutEndTime - $logoutStepStart).TotalSeconds, 1)
                $siteResultsBuffer.Add([PSCustomObject]@{
                    ...
                    Error=$_.Exception.Message; StartTime=$logoutStepStart; EndTime=$logoutEndTime; Timestamp=$logoutEndTime
                    Screenshot=""; DurationSec=$logoutDuration; SiteDurationSec=0; CapturedInfo=""; FooterScreenshot=""
                })
            }
        }
        ...
        $Results.Add([PSCustomObject]@{
            ...
            Error=$_.Exception.Message; StartTime=$siteProcessStart; EndTime=$now; Timestamp=$now
            Screenshot=""; DurationSec=0; SiteDurationSec=[math]::Round(($now - $siteProcessStart).TotalSeconds, 1); CapturedInfo=""; FooterScreenshot=""
        })
    }
    finally {
        # Stamp every row this site produced with the SAME total duration -
        # "per website", not per page/step - now that processing for this
        # site (login through logout) has actually finished.
        if ($siteResultsBuffer.Count -gt 0) {
            $siteProcessEnd = Get-Date
            $siteDurationSec = [math]::Round(($siteProcessEnd - $siteProcessStart).TotalSeconds, 1)
            foreach ($row in $siteResultsBuffer) {
                $row.SiteDurationSec = $siteDurationSec
                $Results.Add($row)
            }
        }
...
# ================= EXCEL REPORT =================
# Columns match the audit-record spec: Site_Name, RowType, URL,
# Status (Pass/Fail/Error/Skipped), Start_Time, End_Time,
# "Duration(Per website)" (same value repeated for every row of that site -
# total login-to-logout time, not this row's own step time), Error.
$ExcelPath = Join-Path $OutputFolder "TestResults.xlsx"
$Results |
    Select-Object @{Name='Site_Name'; Expression={ $_.Site }},
        @{Name='RowType'; Expression={ $_.Page }},
        URL, Status,
        @{Name='Start_Time'; Expression={ if ($_.StartTime) { $_.StartTime.ToString('yyyy-MM-dd HH:mm:ss') } else { '' } }},
        @{Name='End_Time';   Expression={ if ($_.EndTime)   { $_.EndTime.ToString('yyyy-MM-dd HH:mm:ss') }   else { '' } }},
        @{Name='Duration(Per website)'; Expression={ $_.SiteDurationSec }},
        Error |
    Export-Excel -Path $ExcelPath -WorksheetName "Results" -TableName "Results" `
                  -AutoSize -FreezeTopRow -BoldTopRow `
                  -ConditionalText @(
                      New-ConditionalText -Text "Pass"    -BackgroundColor LightGreen
                      New-ConditionalText -Text "Fail"    -BackgroundColor LightYellow
...
$sel.Style = "Title"
$sel.TypeText("Website End-to-End Test Report (Audit)")
$sel.TypeParagraph()
$sel.Style = "Normal"
...
foreach ($siteGroup in $Results | Group-Object Site) {
    ...
    foreach ($r in $siteGroup.Group) {
        ...
        $sel.Style = "Normal"
        $sel.TypeText("URL: $($r.URL)  |  Step duration: $($r.DurationSec) sec  |  Site total: $($r.SiteDurationSec) sec")
        $sel.TypeParagraph()
        $sel.TypeText("Start: $($r.StartTime)  |  End: $($r.EndTime)")
        $sel.TypeParagraph()
...
$summaryColor = if ($failCount -gt 0 -or $errorCount -gt 0) { 'Red' } else { 'Green' }
$summaryLine = "Summary: $passCount Pass, $failCount Fail, $errorCount Error"
