<#
.SYNOPSIS
    Lightweight website check: logs into each site in the config, visits only
    the page(s) listed for that site (not necessarily every page the full
    suite knows about - e.g. just Login + Account Overview), captures
    footer/server-tag screenshots where configured, and writes a single
    Excel status matrix. No Word report is generated.

    Same selector engine, login patterns (two-step, MFA, iframe, shadow DOM),
    and footer-capture logic as Run-WebsiteE2ETests.ps1 - this is a trimmed
    OUTPUT, not a different automation engine. Use a separate, smaller config
    file (e.g. QuickCheck-Config.csv) that only lists the pages you actually
    want checked per site.

.REQUIREMENTS
    Install-Module Selenium     -Scope CurrentUser -Force
    Install-Module ImportExcel  -Scope CurrentUser -Force
    (Microsoft Word is NOT required for this script - no .docx is produced)

.USAGE
    .\Run-QuickSiteCheck.ps1 -ConfigPath .\config\QuickCheck-Config.csv
...
foreach ($group in $AllRows | Group-Object SiteName) {
    ...
    $pageRows = $group.Group | Where-Object { $_.RowType -eq 'Page' }   # only whatever pages THIS config lists - not the full site

    if (-not $loginRow) {
        $Results.Add([PSCustomObject]@{
            Site=$siteName; Page="CONFIG"; URL=""; Status="Error"
            Error="No Login row found in config"; DurationSec=0
        })
        continue
    }
    ...
    $driver = $null
        ...
        # filenames (e.g. "SiteA_00_Login_S09.png"). Not shown as its own
        # Excel column in this lightweight report - just visible in the
        # screenshot filename.
        ...
        # Optional: a dedicated footer screenshot as its own test step, when
        # the footer itself - not the whole page - is what's being checked.
        $loginFooterShotPath = Join-Path $ScreenshotFolder "$siteName`_00_Login_Footer$loginTagSuffix.png"
        Invoke-SeFooterScreenshot -Driver $driver -Path $loginFooterShotPath -CaptureFlag $loginRow.CaptureFooterScreenshot | Out-Null
        ...
        $Results.Add([PSCustomObject]@{
            ...
            DurationSec=$loginDuration
        })

        if ($loginStatus -eq "Failure") { continue }   # skip pages if login didn't work

        # ---- PAGES (only whatever this config lists for the site) ----
        foreach ($page in $pageRows) {
            $status = "Success"; $errMsg = ""; $capturedInfo = ""
            ...
            try {
                ...
                # "S09"/"Sxx" per page - can legitimately differ page to page
                # (different backend instances). Folded into the screenshot
                # filenames below; not shown as its own Excel column here.
                ...
                # where the footer itself is the thing being checked - each
                # page can carry a different server tag, so each gets its
                # own tagged screenshot file.
                Invoke-SeFooterScreenshot -Driver $driver -Path $footerShotPath -CaptureFlag $page.CaptureFooterScreenshot | Out-Null
            }
            catch {
                $status = "Error"
            ...
            $Results.Add([PSCustomObject]@{
                ...
                Error=$errMsg; DurationSec=$pageDuration
            })
        }

        # ---- LOGOUT (only if this config's Login row has a LogoutSelector) ----
        if ($loginRow.LogoutSelector) {
            ...
            try {
                ...
                $Results.Add([PSCustomObject]@{
                    ...
                    Error=""; DurationSec=$logoutDuration
                })
            } catch {
                $logoutDuration = [math]::Round(((Get-Date) - $logoutStepStart).TotalSeconds, 1)
                $Results.Add([PSCustomObject]@{
                    Site=$siteName; Page="LOGOUT"; URL=""; Status="Error"
                    Error=$_.Exception.Message; DurationSec=$logoutDuration
                })
            }
        }
        ...
        $Results.Add([PSCustomObject]@{
            ...
            Error=$_.Exception.Message; DurationSec=0
        })
    }
    finally {
...
# ================= EXCEL REPORT (only) - no Word report in this script =================
$ExcelPath = Join-Path $OutputFolder "QuickCheckResults.xlsx"
# Page holds RowType-style values too (LOGIN, LOGOUT, SITE-LEVEL, CONFIG) as
# well as the configured PageName - same convention as the main script.
$Results |
    Select-Object Site, Page, URL, Status,
        @{Name='TimeTakenSec'; Expression={ $_.DurationSec }},
        Error |
